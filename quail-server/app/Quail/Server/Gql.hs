...
import Quail.Server.Resolve (resolve)
...
rootResolver
    :: forall es
     . (FileSystem :> es, Haxl :> es)
    => RootResolver (Eff es) Event' Query Mutation Subscription
rootResolver =
    RootResolver
        { queryResolver =
            Query
                { isStubAvailable = \(Arg stub) -> lift $ isNothing <$> resolve stub
        ...
        , subscriptionResolver =
            Subscription
                { newUrl = subscribe NewUrl $ pure \(Event _ Url{..}) ->
                    pure
                        Url
                            { stub = pure $ runIdentity stub
                            , target = pure $ runIdentity target
                            }
                }
        }
  where
    ...
    haxl'
        :: (WithOperation o)
        => (a -> b)
        -> GenHaxl () () (Either SomeException a)
        -> Resolver o Event' (Eff es) b
    haxl' f = either (throwError . fromString . show) (pure . f) <=< lift . haxl

gqlApp :: (Typeable es, FileSystem :> es, Haxl :> es) => App Event' (Eff es)
